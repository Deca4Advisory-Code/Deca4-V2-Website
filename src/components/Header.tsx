  ...
  return (
    <header
      className={`fixed flex transition-all duration-500 w-full z-50 pointer-events-auto px-6 md:px-12 top-0 right-0 left-0 items-center justify-between border-b ${
        scrollY > 50
          ? 'bg-white/90 backdrop-blur-md py-4 md:py-5 border-black/10'
      ...
      <Link href="/" className="flex items-center gap-4 cursor-pointer">
        <div className="overflow-hidden transition-all duration-300 group w-32 h-12 relative">
          <img
            ...
            className="transition-all duration-500 w-full  sm:h-12 h-8 sm:mt-0 mt-1 object-contain group-hover:opacity-70" 
            alt="Deca4 Logo"
          />
        </div>
      </Link>
      <button
        className="flex items-center gap-3 hover:text-[#00A99D] transition-colors font-mono text-xs uppercase tracking-widest text-gray-600 sm:mr-0 mr-4 -mt-6"
